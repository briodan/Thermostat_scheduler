...
class T6ProgramConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_misc(self, user_input=None):
        ...
        schema = vol.Schema({
            vol.Required("tolerance_cool", default=DEFAULT_TOLERANCE): vol.All(
                vol.Coerce(float), vol.Range(min=0.5, max=5.0)
            ),
            vol.Required("tolerance_heat", default=DEFAULT_TOLERANCE): vol.All(
                vol.Coerce(float), vol.Range(min=0.5, max=5.0)
            ),
            vol.Required("current_sensor", default=next(iter(sensors))): vol.In(sensors),
            vol.Required("current_temperature", default=DEFAULT_TEMP): vol.All(
                vol.Coerce(float), vol.Range(min=10, max=30)
            ),
            vol.Required("current_target_temperature", default=DEFAULT_TEMP): vol.All(
                vol.Coerce(float), vol.Range(min=10, max=30)
            ),
            vol.Required("adjusted_cool_temperature", default=DEFAULT_TEMP): vol.All(
                vol.Coerce(float), vol.Range(min=10, max=30)
            ),
            vol.Required("adjusted_heat_temperature", default=DEFAULT_TEMP): vol.All(
                vol.Coerce(float), vol.Range(min=10, max=30)
            ),
            vol.Required("thermostat_state", default="idle"): vol.In(["idle", "heat", "cool"])
        })

        return self.async_show_form(step_id="misc", data_schema=schema)
